...
export default function DailyView({
  ...
  const counterInputStyle = { width: '52px', height: '40px', fontSize: '32px', fontWeight: 800, margin: '0 var(--space-xs)', textAlign: 'center', border: 'none', outline: 'none', background: 'var(--primary-bg)', borderRadius: '10px', color: 'var(--text)' } as const;
const months = ["มกราคม", "กุมภาพันธ์", "มีนาคม", "เมษายน", "พฤษภาคม", "มิถุนายน", "กรกฎาคม", "สิงหาคม", "กันยายน", "ตุลาคม", "พฤศจิกายน", "ธันวาคม"];
  ...
  const handleSave = async (dayTypeOverride?: string) => {
    ...
    const payload: any = {
      user_id: userId, year: currentYear, month: currentMonth, day: selectedDay,
      is_work: isWork, shift_time: derivedShift || null
    };
    if (isWork) {
      Object.assign(payload, {
        day_type: dayTypeOverride ?? dayType, odo_in: parseFloat(odoIn) || 0, odo_out: parseFloat(odoOut) || 0,
  ...
  return (
    <div id="view-daily" className="view active">
      ...
      <div style={{
        display: 'flex', gap: 'var(--space-sm)', marginBottom: 'var(--space-lg)', marginTop: 'var(--space-sm)', alignItems: 'center',
        ...
        <button type="button" onClick={() => setShowShiftSelector(true)}
          style={{ padding: 'var(--space-sm) var(--space-lg)', borderRadius: '10px', fontWeight: 700, fontSize: '16px', cursor: 'pointer', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'var(--space-xs)',
        ...
        <label style={{ display: 'flex', gap: 'var(--space-sm)', flex: 1, alignItems: 'center', justifyContent: 'center', cursor: derivedShift === 'หยุด' ? 'default' : 'pointer', opacity: derivedShift === 'หยุด' ? 0.35 : 1, userSelect: 'none' }}
          onClick={() => {
          ...
          <div style={{
            ...
            <div style={{
              position: 'absolute', top: '2px', left: dayType === 'special' ? '22px' : '2px',
      ...
      {isWork ? (
        <>
          {/* Summary Banner */}
          <div className="daily-summary card" style={{ display: 'flex', justifyContent: 'space-around', background: 'var(--primary-bg)', borderRadius: '14px', padding: 'var(--space-md)', marginBottom: 'var(--space-lg)', border: '1px solid var(--border)', boxShadow: '0 2px 8px rgba(0,0,0,0.09)' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '24px', fontWeight: 800, color: 'var(--primary)' }}>{roundCount + helpWork + fixWork} <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--muted)' }}>รอบ</span></div>
            </div>
            <div style={{ width: '1px', background: 'var(--border)' }} />
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--muted)', letterSpacing: '0.3px' }}>จุดส่ง</div>
              <div style={{ fontSize: '24px', fontWeight: 800, color: 'var(--primary)' }}>{pointCount + helpWork + fixWork} <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--muted)' }}>จุด</span></div>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 'var(--space-sm) var(--space-md)', marginBottom: 'var(--space-md)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)', fontSize: '13px', fontWeight: 600, color: 'var(--muted)', lineHeight: 1.6 }}>
              <span style={{ background: 'var(--primary)', color: 'var(--active-date-text, white)', borderRadius: '50%', width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', flexShrink: 0 }}>i</span>
              <span>+ รอบ = จุดอัตโนมัติ · + งานช่วย/แก้ เพิ่มรอบ-จุด ในผลรวม</span>
            </div>
          </div>
          {/* Rounds & Points Card */}
          <div className="card" style={{ display: 'flex', gap: '10px', padding: 'var(--space-md) var(--space-sm)' }}>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--muted)', marginBottom: 'var(--space-sm)', display: 'flex', alignItems: 'center', gap: 'var(--space-xs)' }}>
                <i className="ph-duotone ph-arrows-clockwise i-sm" style={{ color: 'var(--muted)' }}></i> รอบ
              </div>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <button type="button" className="del-btn-small" onClick={() => { setRoundCount(prev => Math.max(0, prev - 1)); setPointCount(prev => Math.max(0, prev - 1)); }}><i className="ph-bold ph-minus"></i></button>
                ...
                <button type="button" className="del-btn-small" onClick={() => { setRoundCount(prev => prev + 1); setPointCount(prev => prev + 1); }}><i className="ph-bold ph-plus"></i></button>
              </div>
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <input type="number" inputMode="numeric" value={pointCount === 0 ? '' : pointCount} placeholder="0"
                  ...
                  style={{ width: '52px', height: '40px', fontSize: '32px', fontWeight: 800, margin: '0 var(--space-xs)', textAlign: 'center', border: 'none', outline: 'none', background: 'var(--primary-bg)', borderRadius: '10px', color: 'var(--text)' }} />
        ...
        <div className="card" style={{ textAlign: 'center', padding: 'var(--space-3xl) var(--space-xl)' }}>
          ...
          <h3 style={{ fontSize: '20px', fontWeight: 800, color: leaveType === 'sick' ? '#f1c40f' : leaveType === 'personal' ? '#3498db' : 'var(--primary)', marginBottom: '4px' }}>
