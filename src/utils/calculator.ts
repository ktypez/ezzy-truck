export const calculateSalary = (logs: any[], daysInMonth?: number) => {
  ...
  const othersGross = (roundInc + pointInc + otInc + foodInc + phoneInc + diligenceInc + extraInc) - lateDed - leaveDed;
  ...
  return {
    base, roundInc, pointInc, otInc, foodInc, phoneInc, diligenceInc, extraInc, lateDed, leaveDed,
    salaryBase, salaryBaseTax, othersGross, othersTax, totalGross, totalTax, netIncome,
    totalRounds, totalPoints, totalOT, x2Days, workDays, sickDays, personalDays, sickExcess, personalExcess
  };
};
